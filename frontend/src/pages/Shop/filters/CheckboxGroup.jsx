    ...
    <div className="mb-2">
      <div className="flex justify-between items-center mb-2">
        <h3 className="font-bold text-slate-800 text-xs flex items-center gap-1.5">
          {title}
          {count > 0 && (
            <span className="w-4 h-4 bg-teal-600 text-white text-[9px] font-bold rounded-full flex items-center justify-center">
              {count}
            </span>
          )}
        </h3>
        {count > 0 && (
          <button onClick={() => setSelectedState([])}
            className="text-[10px] font-bold text-teal-600 hover:text-teal-700 cursor-pointer transition-colors">
            Clear
          </button>
        )}
      </div>
        ...
        {options.map(opt => (
          <label key={opt}
            ...
            <span className={`text-xs font-medium transition-colors ${
              selectedState.includes(opt) ? 'text-teal-800 font-bold' : 'text-slate-600 group-hover:text-slate-900'
            }`}>{opt}</span>
