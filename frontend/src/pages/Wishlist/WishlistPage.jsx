...
function resolveImage(url) {
  if (!url) return 'https://placehold.co/200x200/fdf4f0/0f766e?text=🍽️';
        ...
        {items.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {items.map(product => (
                <div key={product.id} className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all group">
                  <div className="relative" style={{ aspectRatio: '1' }}>
                    <img src={resolveImage(product.image_url)} alt={product.name}
                      className="w-full h-full object-cover"
                      onError={e => { e.target.src = 'https://placehold.co/200x200/fdf4f0/0f766e?text=🍽️'; }} />
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-black text-teal-600 text-base">₹{product.price}</span>
